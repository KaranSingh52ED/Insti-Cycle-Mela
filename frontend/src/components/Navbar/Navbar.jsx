    ...
    return (
        <div className='flex justify-between items-center p-8 bg-gray-200 fixed top-0 left-0 w-full h-12 sm:gap-10'>
            <div className='flex w-1/3'>
                <img src="https://tse3.mm.bing.net/th?id=OIP.zN59XZkqlJjGp8P5-viNuwHaHa&pid=Api&P=0&h=180" alt="" className='w-12 h-12 rounded-full' />
            </div>
            ...
            <div className='flex space-x-4 gap-5 items-center justify-end w-1/3'>
                ...
                {state.isAuthenticated ? (
                    ...
                    <button onClick={handleLogout} className='text-lg border-b-2 border-black hover:border-none p-1'>Logout</button>
                    </>
                ) : (
                    <Link to="/login" className='text-lg border-b-2 border-black hover:border-none p-1'>Login</Link>
                )}
                {location.pathname !== "/" && (
                    <Link to="/" className='text-lg border-b-2 border-black hover:border-none p-1'>Home</Link>
                )}
                <Link to="/sell" className='flex items-center font-bold justify-center gap-2 bg-blue-600 p-2 px-3 rounded text-white'><FaPlus />Sell</Link>
            </div>
        </div>
    )
}
